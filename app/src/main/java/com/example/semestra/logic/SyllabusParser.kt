...
class SyllabusParser {
    ...
    fun parseText(text: String): List<ExamEvent> {
        val events = mutableListOf<ExamEvent>()
        val lines = text.lines()

        for ((index, line) in lines.withIndex()) {
            val examMatch = EXAM_LABEL_REGEX.find(line) ?: continue
            val searchWindow = lines.drop(index).take(3).joinToString(" ")
            val dateMatch = DATE_REGEX.find(searchWindow) ?: continue
        ...
        return events.distinctBy { it.eventDate to it.examTitle }
    }
